...
data = {
    ...
    'SystolicBP': np.zeros(total_samples, dtype=np.int64),
    'DiastolicBP': np.zeros(total_samples, dtype=np.int64),
...
n_samples_per_category = {
    'No Impairment': int(total_samples * 0.25),
    'Very Mild': int(total_samples * 0.25),
    'Mild': int(total_samples * 0.25),
    'Moderate': int(total_samples * 0.25)
}

mmse_values = []
for category, (min_val, max_val) in mmse_ranges.items():
    n_samples = n_samples_per_category[category]
    # Use uniform distribution to avoid clustering at edges
...
def correlated_continuous(mmse, min_val, max_val, strength, inverse=False):
    scaled_mmse = (mmse - 0) / (29.99 - 0)
    if inverse:
        scaled_mmse = 1 - scaled_mmse
    strength_arr = np.full_like(scaled_mmse, strength, dtype=float)
    ...
    std = (max_val - min_val) / 10  # Even tighter std to avoid outliers
...
def colesterol_realista(mmse, age, min_val, max_val):
    ...
    # Use uniform distribution to avoid outliers
    values = np.random.uniform(mean - (max_val - min_val) / 4, mean + (max_val - min_val) / 4, len(mean))
    return np.clip(values, min_val, max_val)

# Grupo 1
data['Age'] = np.clip(correlated_continuous(data['MMSE'], 50, 90, 0.7, inverse=True).astype(np.int64), 50, 90)
data['FamilyHistoryAlzheimers'] = correlated_binary(0.5, data['MMSE'], 0.5, inverse=True)
...
data['FunctionalAssessment'] = correlated_continuous(data['MMSE'], 0, 9.99, 0.7)
...
data['SystolicBP'] = np.clip(correlated_continuous(data['MMSE'], 90, 180, 0.6, inverse=True).astype(np.int64), 90, 180)
data['DiastolicBP'] = np.clip(correlated_continuous(data['MMSE'], 60, 110, 0.6, inverse=True).astype(np.int64), 60, 110)
data['CholesterolTotal'] = colesterol_realista(data['MMSE'], data['Age'], 160, 240)
data['CholesterolLDL'] = colesterol_realista(data['MMSE'], data['Age'], 80, 160)
data['CholesterolTriglycerides'] = colesterol_realista(data['MMSE'], data['Age'], 60, 200)
...
data['Depression'] = correlated_binary(0.4, data['MMSE'], 0.3, inverse=True)
...
data['AlcoholConsumption'] = np.random.uniform(0.0020, 19.9893, total_samples)  # Uniform to avoid outliers
data['BMI'] = correlated_continuous(data['MMSE'], 15, 39.99, 0.5, inverse=True)
data['CholesterolHDL'] = correlated_continuous(data['MMSE'], 30, 100, 0.4)
...
data['Diagnosis'] = (data['MMSE'] < 27).astype(np.int64)

# Criar DataFrame
df = pd.DataFrame(data)

# Post-processing: Clip numerical columns to IQR bounds to remove outliers
numerical_columns = [
...
# Garantir que todos os valores ainda respeitem os limites originais
df['Age'] = np.clip(df['Age'], 50, 90).astype(np.int64)
df['SystolicBP'] = np.clip(df['SystolicBP'], 90, 180).astype(np.int64)
df['DiastolicBP'] = np.clip(df['DiastolicBP'], 60, 110).astype(np.int64)
df['CholesterolTotal'] = np.clip(df['CholesterolTotal'], 160, 240)
df['CholesterolLDL'] = np.clip(df['CholesterolLDL'], 80, 160)
df['CholesterolTriglycerides'] = np.clip(df['CholesterolTriglycerides'], 60, 200)
df['CholesterolHDL'] = np.clip(df['CholesterolHDL'], 30, 100)
df['BMI'] = np.clip(df['BMI'], 15, 39.99)
df['AlcoholConsumption'] = np.clip(df['AlcoholConsumption'], 0.0020, 19.9893)
df['PhysicalActivity'] = np.clip(df['PhysicalActivity'], 0.0036, 9.9874)
df['DietQuality'] = np.clip(df['DietQuality'], 0, 9.99)
df['SleepQuality'] = np.clip(df['SleepQuality'], 4, 9.99)
df['ADL'] = np.clip(df['ADL'], 0, 9.99)
df['FunctionalAssessment'] = np.clip(df['FunctionalAssessment'], 0, 9.99)
df['MMSE'] = np.clip(df['MMSE'], 0, 30)

# Salvar o dataset
df.to_csv('alzheimers_disease_data.csv', index=False)
print("Dataset realista sem outliers gerado e salvo como 'alzheimers_disease_data.csv'")